...
function titleFormatter(value, row) {
    ...
    return `<div class="text-truncate clickable-title" style="max-width: 200px; cursor: pointer; color: #007bff;" 
                title="${value}" onclick="showDetail('/browse/validation-detail?id=',${row.id})">${value}</div>`;
}
...
function initNoUiSlider() {
    ...
    window.yearSlider = noUiSlider.create(slider, {
        ...
        pips: {
            mode: 'values',
            values: [2005, 2010, 2015, 2020, currentYear],
            density: 4,
            format: {
                to: function (value) {
...
$(document).ready(function () {
    ...
    initBootstrapTable('/browse/validation-data');
    ...
    // 检查URL参数并打开对应模态框
    //validation?modal=detail&detail=1
    const modalId = urlParams.get('modal');
    const detailId = urlParams.get('detail');
    if (modalId === 'detail' && detailId) {
        // 延迟执行，等待页面完全加载
        setTimeout(() => {
            showDetail('/browse/validation-detail?id=', detailId);
        }, 1000);
    }
});
